...
;(function($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    buildOuterContainer: function() {
      var $outerContainer = $('<div />').addClass('outer-container').attr('id', 'stories-feed-container');
    ...
    buildTopStoryElement: function() {
      ...
      $.ajax({
        ...
        success: function(html) {
          var $outerContainer = $('#stories-feed-container');
          var $storiesFeed = self.buildStoriesFeedElement.call(self);
    ...
    buildStoriesFeedElement: function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          var topStoryId = $('.feed-column.top-story article').attr('id');
          ...
          if ( data.length ) {
            ...
            var duplicateStoryElement = $storiesElement.find('article[id=' + topStoryId + ']')
            var lastStoryElement = $storiesElement.find('article[id=' + dataObj[dataObj.length - 1].id + ']');
            duplicateStoryElement ? duplicateStoryElement.remove() : lastStoryElement.remove();
