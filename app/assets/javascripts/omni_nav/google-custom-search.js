   ...
   */
	var SearchComponent = function(containerId) {
		...
		var init = function() {
			...
			$('td#gs_tti50').prepend('<span class="sr-only" id="search-label"> search </span>');
			$('input#gsc-i-id1').attr('aria-labelledby', 'search-label');

			// fix empty label accessibility error
			// $('input.gsc-input').prepend('<label for>');
			// $('.gsc-input label').attr('for', 'Search', 'id', 'cu-search');
			// $('.gsc-input label').append('<span class="sr-only" id="search-box"> search </span>');
			$('input#gsc-i-id1').attr('aria-labelledby', 'gsc-iw-id1');
		...
		var onDocumentClick = function(e) {
			if (
				$(e.target).is($searchResults, $searchBox) ||
